...
class DioClient {
  ...
  final String databaseDatabase = dotenv.env['DATABASE_DATABASE'] ??
      "database_database";
  final String databasePassword = dotenv.env['DATABASE_PASSWORD'] ??
      "database_password";

  Options get options =>
      Options(headers: {
        'Authorization': 'Basic ${base64Encode(
            utf8.encode('$databaseUser:$databasePassword'))}',
  ...
  Future<Map<String, dynamic>> insert(String table, List<String> params,
      List<String> args) async {
    // TODO: this may be dangerous
    String questionMarkListParams = "(${params.map((e) => "?").join(",")})";
    String questionMarkList = "(${args.map((e) => "?").join(",")})";
    String query = "INSERT INTO $table $questionMarkListParams VALUES $questionMarkList";
    Response response = await sendRequest(query, args, 'exec');

    return response.data;
  }

  Future<Map<String, dynamic>> update(String table, List<String> attrs,
      List<String> values, int id) async {
  ...
  Future<Response> sendRequest(String query, List<String> args,
      String urlOption) async {
